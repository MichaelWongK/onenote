...
![img](http://www.micheal.wang:10020/mongo/read/5f4debf89c5fd27c4ff5d8f8)
...
在其它数据库系统比如 SQL Server 默认的隔离级别就是提交读，已提交读隔离级别就是在事务未提交之前所做的修改其它事务是不可见的。

### 可重复读（REPEATABLE READ） 

保证同一个事务中的多次相同的查询的结果是一致的，比如一个事务一开始查询了一条记录然后过了几秒钟又执行了相同的查询，保证两次查询的结果是相同的，可重复读也是 mysql 的默认隔离级别。
...
![image-20200901153408172](http://www.micheal.wang:10020/mongo/read/5f4df97e9c5fd27c4ff5d8fc)
